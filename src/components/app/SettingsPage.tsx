...
import { ArrowLeft, Sun, Moon, Bell, BellOff, Lock, Globe, Database, FileDown, FileSpreadsheet } from "lucide-react";
  ...
  return (
    <div className="px-4 pb-6 space-y-5">
      ...
      <div className="p-4 rounded-2xl bg-card border border-border space-y-3">
        ...
        <button className="w-full py-2.5 rounded-xl bg-accent text-xs font-medium text-foreground text-center">
          Change Password
        </button>
        <div className="flex items-center justify-between">
      ...
      <div className="p-4 rounded-2xl bg-card border border-border space-y-3">
        ...
        <p className="text-[10px] text-muted-foreground">Backup and export your investment data</p>
        <div className="space-y-2">
          <button onClick={() => handleExport("Fragments Data")} className="w-full py-2.5 rounded-xl bg-accent text-xs font-medium text-foreground text-center">
            Backup Fragments Data
          </button>
          <button onClick={() => handleExport("Uploaded Documents")} className="w-full py-2.5 rounded-xl bg-accent text-xs font-medium text-foreground text-center">
            Backup Uploaded Documents
          </button>
          <button onClick={() => handleExport("One Property Documents")} className="w-full py-2.5 rounded-xl bg-accent text-xs font-medium text-foreground text-center">
            Backup One Property Documents
          </button>
        </div>
        <div className="pt-2 border-t border-border">
          <p className="text-[10px] text-muted-foreground mb-2">Export Options</p>
          <div className="flex gap-2">
            <button onClick={() => handleExport("PDF")} className="flex-1 py-2.5 rounded-xl bg-primary/10 text-xs font-semibold text-primary flex items-center justify-center gap-1.5">
              <FileDown className="w-3.5 h-3.5" /> Export PDF
            </button>
            <button onClick={() => handleExport("Excel")} className="flex-1 py-2.5 rounded-xl bg-teal/10 text-xs font-semibold text-teal flex items-center justify-center gap-1.5">
              <FileSpreadsheet className="w-3.5 h-3.5" /> Export Excel
            </button>
          </div>
          <p className="text-[9px] text-muted-foreground mt-2">Includes: Fragment details, Ownership documents, KYC documents, Power of Attorney</p>
        </div>
      </div>
    </div>
  );
